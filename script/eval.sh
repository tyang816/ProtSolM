...
model_name=feature_norm_pp_"$pooling_method"_k"$K"_h"$H"_lr"$lr"
...
CUDA_VISIBLE_DEVICES=1 python eval.py \
    ...
    --test_result_dir result/$project_name/protssn_k"$K"_h"$H"/$model_name/ExternalTest \
...
CUDA_VISIBLE_DEVICES=1 python eval.py \
    ...
    --test_result_dir result/$project_name/protssn_k"$K"_h"$H"/$model_name/test \
